param(
    [string]$Src = "$PSScriptRoot\..\themes",
    [string]$Build = "$PSScriptRoot\..\build",
    [string]$PaletteFileName = "palette.json"
)

$Colors = Get-Content $(Join-Path $Src $PaletteFileName) | Out-String | ConvertFrom-Json
$Files = Get-ChildItem $Src 

foreach($File in $Files) {
    if($File.Name -eq $PaletteFileName) {
        continue
    }

    $Theme = Get-Content $File.FullName | Out-String
    ...
    $OutFile = $File.Name.Replace("color-template.json", "color-theme.json")
    $OutFilePath = Join-Path $Build $OutFile
